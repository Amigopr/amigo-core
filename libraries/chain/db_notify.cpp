...
struct get_impacted_account_visitor
{
   ...
   void operator()( const delay_transfer_operation& op )
   {
      for(auto it=op.to_list.begin(); it!=op.to_list.end(); it++)
      {
   ...
   void operator()( const module_cfg_operation& op ) {}
   ...
   void operator()( const token_publish_operation& op )
   {
      _impacted.insert( op.issuer );
   }
   void operator()( const token_buy_operation& op )
   {
      _impacted.insert( op.buyer );
   }
   void operator()( const token_event_operation& op )
   {
      _impacted.insert( op.oper );
   }
   void operator()( const token_update_operation& op )
   {
      _impacted.insert( op.oper );
   }
   void operator()( const operation_wrapper_operation& op ) {}
};
      ...
      switch( (object_type)obj->id.type() )
      {
        ...
        case account_object_type:{
          ...
          break;
           ...
           break;
        } case module_cfg_object_type:{
        	 break;
        } case token_object_type:{
           break;
        } case token_buy_object_type:{
           break;
        } case token_event_object_type:{
        	 break;
        } case delay_transfer_object_type:{
           const auto& aobj = dynamic_cast<const delay_transfer_object*>(obj);
   ...
   else if( obj->id.space() == implementation_ids )
   {
      switch( (impl_object_type)obj->id.type() )
      {
             ...
             case impl_fba_accumulator_object_type:
              break;
             case impl_token_statistics_object_type:
              break;
             case impl_word_object_type:
              break;
             case impl_operation_blacklist_object_type:
              break;
             case impl_delay_transfer_unexecuted_object_type:
              break;
      }
   }
...
bool need_to_notify(const object_id_type& object_id)
{
  uint8_t space = object_id.space();
  uint8_t type = object_id.type();
  // ilog("need_to_notify. space: ${space}, type: ${type}", ("space", space)("type", type));
  //bool needToNotify = true;
  bool needToNotify = (
    !(space == protocol_ids && type == token_buy_object_type)
    );

  if (!needToNotify)
  {
    ilog("no need to notify: ${object_id}", ("object_id", object_id));
  }
  return needToNotify;
}
...
{ try {
   if( _undo_db.enabled() ) 
   {
      ...
      if( !new_objects.empty() )
      {
        ...
        ilog("new_accounts_impacted: ${new_accounts_impacted}, new_ids: ${new_ids}", 
          ("new_accounts_impacted", new_accounts_impacted)("new_ids", new_ids));
      ...
      if( !changed_objects.empty() )
      {
        ...
        {
          //filter objects not need to notify
          if (need_to_notify(item.first))
          {
        ...
        ilog("changed_accounts_impacted: ${changed_accounts_impacted}, changed_ids: ${changed_ids}", 
          ("changed_accounts_impacted", changed_accounts_impacted)("changed_ids", changed_ids));
      ...
      if( !removed_objects.empty() )
      {
        ...
        ilog("removed_accounts_impacted: ${removed_accounts_impacted}, removed_ids: ${removed_ids}", 
          ("removed_accounts_impacted", removed_accounts_impacted)("removed_ids", removed_ids));
